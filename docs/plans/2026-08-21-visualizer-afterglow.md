...
- Fade radar samples linearly from maximum alpha `.28f` to zero, draw them in `PhosphorDim` with the existing `1f` sweep width, oldest first, and retain the existing live `PhosphorBright` arm at alpha `.8f`.
...
- Draw only ghost segments strictly above the live bar, in `PhosphorDim` with maximum alpha `.30f`. Keep existing live `Phosphor` segments and the current `PhosphorBright` peak line unchanged and above the ghost layer.
